-- ============================================================
-- Focus Lily — Full Schema for Supabase
-- Run this in: Supabase Dashboard → SQL Editor → New Query
-- ============================================================

-- ============================================================
-- 1. CORE: profiles table + trigger
-- ============================================================
...
-- ============================================================
-- 3. PROFILE PUBLIC: username, rank, avatar_url, public_profile
-- ============================================================
...
-- ============================================================
-- 5. CHAT SYSTEM: presence, friends, messages
-- ============================================================
ALTER TABLE profiles
...
-- pg_trgm for username/display_name search
...
GRANT EXECUTE ON FUNCTION
  get_following_ids(uuid), get_follower_ids(uuid), get_follow_count(uuid),
  get_is_following(uuid, uuid), get_mutual_following_ids(uuid, uuid)
  TO authenticated;

-- ============================================================
-- 7. RLS for all chat tables
-- ============================================================
...
BEGIN
  ...
  WITH live AS (
    ...
    FROM realm_presence
    WHERE room_key = p_room_key AND user_id <> me
  ...
  FROM candidates c LEFT JOIN live l ON l.instance = c.instance
...
GRANT EXECUTE ON FUNCTION
  assign_realm_instance(text, int), realm_heartbeat(text, int),
  leave_realm_presence(), realm_occupancy(text)
  TO authenticated;
...
DECLARE
  ...
  slug text; code text; tries int := 0; row realms;
BEGIN
  IF me IS NULL THEN RAISE EXCEPTION 'unauthenticated'; END IF;
  ...
  INSERT INTO realms (code, name, owner_id, visibility, player_limit)
    VALUES (code, nm, me, vis, lim) RETURNING * INTO row;
...
-- get_realm_by_code (with ban check)
...
GRANT SELECT ON realms, realm_members, realm_bans TO authenticated;
GRANT EXECUTE ON FUNCTION
  create_realm(text, text, int), get_realm_by_code(text),
  update_realm(uuid, text, text, int), close_realm(uuid), list_public_realms(),
  kick_member(uuid, uuid), ban_member(uuid, uuid), unban_member(uuid, uuid),
  mute_member(uuid, uuid), unmute_member(uuid, uuid),
  transfer_realm_ownership(uuid, uuid), list_realm_members(uuid)
  TO authenticated;
...
GRANT EXECUTE ON FUNCTION
  claim_session(uuid, text), session_heartbeat(uuid), release_session()
  TO authenticated;
...
CREATE POLICY "Anyone can upload an avatar"
  ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'avatars' AND (SELECT auth.uid()) IS NOT NULL);

CREATE POLICY "Note images are publicly accessible"
  ON storage.objects FOR SELECT USING (bucket_id = 'note-images');

CREATE POLICY "Anyone can upload a note image"
  ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'note-images' AND (SELECT auth.uid()) IS NOT NULL);
