      ...
      <group ref={bind('hips')} position={[0, P.hipsY, 0]}>
        ...
        <group ref={bind('spine')} position={[0, 0.04, 0]}>
          <mesh geometry={torsoGeo([
            ...
            { y: P.spineLen + P.chestLen * 0.45, hw: P.chestW, hd: P.torsoD },
        ...
        <Arm side="L" bind={bind} P={P} skin={skin} topM={topM} isSleeved={isSleeved} hand={config.accessories} />
        <Arm side="R" bind={bind} P={P} skin={skin} topM={topM} isSleeved={isSleeved} hand={config.accessories} />
      </group>
...
function Hair({ config, P, hairM }: { config: AvatarConfig; P: Proportions; hairM: Mat }) {
  ...
  switch (config.hair) {
    ...
    case 'ponytail': return <group>{texturedCap([r * 1.09, r * 1.03, r * 1.09], 0.15, 0.11)}<mesh geometry={sphereGeo(1)} material={hairM} scale={[r * 0.9, r * 0.45, r * 0.9]} position={[0, cy + r * 0.4, -r * 0.1]} /><Strand m={hairM} len={r * 2.5} rTop={r * 0.4} rBot={r * 0.15} p={[0, cy + r * 0.45, -r * 0.85]} rot={[-0.25, 0, 0]} />{fringe(0.82)}</group>
...
function Arm({ side, bind, P, skin, topM, isSleeved, hand }: {
  side: 'L' | 'R'; bind: (n: BoneName) => (g: Group | null) => void
  P: Proportions; skin: Mat; topM: Mat; isSleeved: boolean; hand?: EquippedAccessories
}) {
  ...
  return (
    <group ref={bind(upper)} position={[sign * P.shoulderW, 0.04 + P.spineLen + P.chestLen * 0.65, 0]}>
      {/* Upper arm — bicep/deltoid bulges */}
      <mesh geometry={latheGeo([
        [P.elbowR, -P.upperArm],
        [P.elbowR * 1.15, -P.upperArm * 0.82],
        [P.elbowR * 1.35, -P.upperArm * 0.55],
        [P.shoulderR * 1.2, -P.upperArm * 0.3],
        [P.shoulderR * 1.8, -P.upperArm * 0.12],
        [P.shoulderR * 2.1, -P.upperArm * 0.04],
        [P.shoulderR * 1.85, 0],
      ])} material={armM} castShadow />

      {/* Shoulder ball — connects arm to torso visually */}
      <mesh geometry={sphereGeo(1)} material={armM} scale={[P.shoulderR * 1.6, P.shoulderR * 1.6, P.shoulderR * 1.6]} position={[0, 0, 0]} castShadow />
...
function Leg({ side, bind, P, skin, botM, shoeM, shoeAccent, config }: {
  ...
  return (
    <group ref={bind(upper)} position={[sign * P.hipW, P.hipsY - 0.02, 0]}>
