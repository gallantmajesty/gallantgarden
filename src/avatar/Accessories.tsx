...
export function AccessoryModel({ id }: { id: AccessoryId }) {
  ...
  case 'tablet': {
    const shellOuter = tm('#5b3a22', 0.5, 0, 'wood')
    const shellRim = tm('#3a2616', 0.55, 0, 'wood')
    const screenGlass = m('#1a120a', 0.3)
    const screenOn = m('#2a1c10', 0.4)
    const homeBtn = m('#241510', 0.35)
    const stylusBody = tm('#caa24a', 0.4, 0.1, 'leather')
    const stylusTip = m('#241510', 0.2, 0.5)
    const apps = ['#c9924a', '#d99a4e', '#a06a3a', '#8a5a37', '#e0b878', '#b87333', '#a06a3a', '#d99a4e']
    return (
      <group rotation={[0.04, 0, 0]}>
        {/* ---- rounded tablet body via lathe profile ---- */}
        <mesh
          geometry={latheGeo([
            [0.02, 0.01],
            [0.015, 0.12],
        ...
        {/* ---- screen glass layer ---- */}
        <mesh geometry={boxGeo(0.27, 0.44, 0.003)} material={screenGlass} position={[0, 0.44, 0.002]} />
        {/* active display area */}
        <mesh geometry={boxGeo(0.258, 0.42, 0.001)} material={screenOn} position={[0, 0.44, 0.004]} />
        {/* wallpaper */}
        <mesh geometry={boxGeo(0.252, 0.408, 0.001)} material={m('#7a4a2b', 0.5)} position={[0, 0.44, 0.005]} />
        {/* app grid */}
        {apps.map((c, i) => (
          <mesh key={i} geometry={boxGeo(0.05, 0.05, 0.0012)} material={m(c, 0.45)} position={[-0.08 + (i % 4) * 0.055, 0.475 - Math.floor(i / 4) * 0.065, 0.006]} />
        ))}
        {/* single sticky note at bottom middle, with the one dot inside it */}
        <group position={[0, 0.06, 0.007]} rotation={[0, 0, 0.08]}>
          {/* note paper */}
          <mesh geometry={boxGeo(0.08, 0.08, 0.002)} material={tm('#f0e2bf', 0.9, 0, 'paper')} />
          {/* curled corner */}
          <mesh geometry={boxGeo(0.02, 0.02, 0.002)} material={m('#e6d2a0', 0.9)} position={[0.03, -0.03, 0.001]} rotation={[0, 0, 0.4]} />
          {/* the one dot inside the note */}
          <mesh geometry={sphereGeo(0.014)} material={homeBtn} position={[0, 0.005, 0.002]} />
        </group>

        {/* ---- front camera (top center bezel) ---- */}
        <mesh geometry={sphereGeo(0.012)} material={m('#1a120a', 0.3)} position={[0, 0.86, 0.014]} />
        <mesh geometry={sphereGeo(0.006)} material={m('#241510', 0.5)} position={[0, 0.86, 0.017]} />

        {/* ---- side power button ---- */}
        <mesh geometry={boxGeo(0.004, 0.04, 0.008)} material={shellRim} position={[0.146, 0.56, 0]} />
        {/* volume up/down */}
        <mesh geometry={boxGeo(0.004, 0.03, 0.008)} material={shellRim} position={[-0.146, 0.58, 0]} />
        <mesh geometry={boxGeo(0.004, 0.03, 0.008)} material={shellRim} position={[-0.146, 0.52, 0]} />

        {/* ---- improved slim stylus ---- */}
        <group position={[0.06, 0.49, 0.06]} rotation={[0.5, -0.3, 0.6]}>
          {/* shaft */}
          <mesh geometry={taperGeo(0.009, 0.007, 0.32)} material={stylusBody} />
          {/* tip nib */}
          <mesh geometry={sphereGeo(0.006)} material={stylusTip} position={[0, -0.16, 0]} scale={[1, 0.6, 1]} />
          {/* clip */}
          <mesh geometry={boxGeo(0.002, 0.12, 0.003)} material={m('#a88a5a', 0.3, 0.3)} position={[0.011, 0.08, 0]} />
        </group>
      </group>
    )
  }
