...
//  • opening a DM subscribes to a channel and listens for broadcast events;
//  • sending a message persists to DB AND broadcasts to the channel;
//  • a SLOW poll (8s) keeps summaries fresh;
//  • a fallback poll runs if realtime disconnects.
...
function subscribeToChat(conversationId: string, onMessage: (msg: Message) => void) {
  ...
  try {
    activeChannelObj = supabase.channel(channelName)
...
export const useChat = create<ChatState>((set, get) => ({
  ...
  send: async (body) => {
    ...
    if (activeChannelObj) {
      try {
        await activeChannelObj.send({ type: 'broadcast', event: 'chat_message', payload: msg })
