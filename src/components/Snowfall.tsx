...
export default function Snowfall(props: SnowfallProps) {
  ...
  const cfgRef = useRef({ color, wind: cfg.wind, windVariation: cfg.windVariation })
  cfgRef.current = { color, wind: cfg.wind, windVariation: cfg.windVariation }
  ...
  useEffect(() => {
    ...
    function loop(t: number) {
      const { wind: wBase, windVariation: wVar } = cfgRef.current
      for (let i = 0; i < flakes.length; i++) {
        const f = flakes[i]
        f.y += f.vy * dirSign
