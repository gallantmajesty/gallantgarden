...
export async function assignInstance(roomKey: string, capacity = REALM_CAPACITY): Promise<number> {
  try {
    const { data, error } = await insforge.database.rpc('assign_realm_instance', {
      p_room_key: roomKey,
      p_capacity: capacity,
    })
...
export async function occupancy(roomKey: string): Promise<InstanceOccupancy[]> {
  try {
    const { data, error } = await insforge.database.rpc('realm_occupancy', { p_room_key: roomKey })
...
async function heartbeat(roomKey: string, instance: number): Promise<void> {
  try {
    await insforge.database.rpc('realm_heartbeat', { p_room_key: roomKey, p_instance: instance })
  } catch {
...
export async function leavePresence(): Promise<void> {
  try {
    await insforge.database.rpc('leave_realm_presence', {})
  } catch {
