...
export async function syncStartJourney(userId: string | null, a: StartArgs): Promise<void> {
  if (!userId) return
  try {
    await supabase.rpc('start_train_journey', {
      p_line_id: a.lineId,
...
export async function syncHeartbeat(userId: string | null, activeFocusSec: number): Promise<void> {
  if (!userId) return
  try {
    await supabase.rpc('train_journey_heartbeat', { p_active_focus_sec: Math.round(activeFocusSec) })
  } catch {
    /* ignore */
  }
}
...
export async function syncCompleteJourney(userId: string | null, e: JournalEntry): Promise<void> {
  if (!userId) return
  try {
    await supabase.rpc('complete_train_journey', {
      p_line_id: e.lineId,
      p_minutes: e.minutes,
...
export async function syncAbandonJourney(userId: string | null): Promise<void> {
  if (!userId) return
  try {
    await supabase.rpc('abandon_train_journey', {})
  } catch {
    /* ignore */
  }
}
...
export async function fetchActiveJourney(userId: string | null): Promise<PersistedJourney | null> {
  ...
  try {
    const { data, error } = await supabase.rpc('get_active_train_journey', {})
